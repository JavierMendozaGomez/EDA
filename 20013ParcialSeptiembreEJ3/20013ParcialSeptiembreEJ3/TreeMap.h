...
public:
	...
	const Clave &atValor(const Valor &valor) const {
		Nodo *p = buscaValor(_ra, valor);
		if (p == nullptr)
			throw std::out_of_range("No se encuentra la ordenada");
		return p->_clave;
	}
	/**
	...
	ConstIterator consultaK(const int &n)const {
		int i = 0;
		...
		if (it == nullptr)
			throw std::invalid_argument("No existe la kesima tarea");
		return it;
	}
	/**
...
private:
	...
	static Nodo *buscaValor(Nodo *p, const Valor & valor) {
		if (p == nullptr)
			return nullptr;
		if (p->_valor == valor)
			return p;
		if (valor < p->_valor)
			return buscaValor(p->_iz, valor);
		else
			return buscaValor(p->_dr, valor);
	}

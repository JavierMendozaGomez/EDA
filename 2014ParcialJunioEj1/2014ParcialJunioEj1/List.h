...
public:
	...
	*/
	...
	void insertaNodo(const T &elem, const int & pos) {
		if (pos == 0) {
			Nodo * nuevoNodo = new Nodo(nullptr, elem, _prim);
			_prim->_ant = nuevoNodo;
			_prim = nuevoNodo;
		}
		else if (pos == _numElems) {
			Nodo * nuevoNodo = new Nodo(_ult, elem, nullptr);
			_ult->_sig = nuevoNodo;
			_ult = nuevoNodo;
		}
		else {
			...
			Nodo *anterior = aux->_ant;
			Nodo * nuevoNodo = new Nodo(anterior, elem, aux);
			aux->_ant = nuevoNodo;
			anterior->_sig = nuevoNodo;
		}
		_numElems++;
	}
	/**

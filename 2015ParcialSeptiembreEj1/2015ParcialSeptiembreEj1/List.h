...
public:
	...
	*/
	...
	void intercalaLista() {
		if (_numElems >= 3) {
			...
			while (aux != _ult) {
				...
				_ult->_sig = aux;
				aux->_ant = _ult;
				ultimoAux->_sig = nullptr;
				_ult = ultimoAux;
				aux = aux->_sig;
			}
		}
	}
	void intercalaListaV2() {
		int i = 0;
		Nodo *aux = _prim;
		Nodo  *siguiente, *ultimoAux;
		while (_numElems - i >= 3) {//No va a haber intercambio si hay menos de 3 elementos
			ultimoAux = _ult->_ant;
			ultimoAux->_sig = nullptr;
			siguiente = aux->_sig;
			aux->_sig = _ult;
	...
	void invierte() {
		...
		Nodo * aux = _prim;
		Nodo * auxFin = _ult;
		Nodo * anteriorAux, *siguienteAux, *anteriorFin, *siguienteFin;
		
		while (aux != auxFin && _numElems - i >= 2) { //Solo intercambia cuando hay 2 elementos
			anteriorAux = aux->_ant;
			siguienteAux = aux->_sig;
			siguienteFin = auxFin->_sig;
			anteriorFin = auxFin->_ant;
			
			auxFin->_ant = anteriorAux;
			auxFin->_sig = siguienteAux;
			if (anteriorAux != nullptr)
				anteriorAux->_sig = auxFin;
			else
				_prim = auxFin;
			siguienteAux->_ant = auxFin;
			if (siguienteFin != nullptr)
				siguienteFin->_ant = aux;
			else
				_ult = aux;
			aux->_sig = siguienteFin;
			aux->_ant = anteriorFin;
			anteriorFin->_sig = aux;
			aux = siguienteAux;
			auxFin = anteriorFin;
			i = i + 2;
		}

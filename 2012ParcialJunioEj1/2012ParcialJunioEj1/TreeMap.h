...
public:
	...
	const Clave &siguienteClave(const Clave & clave) const {
		Nodo * p = buscaSigAux(_ra, clave,nullptr);
		if (p == nullptr)
			throw std::domain_error("clave erronea");
		return p->_clave;
	}
...
private:
	...
	static Nodo * buscaSigAux(Nodo *p, const Clave & clave,Nodo *ascendiente) {
		if (p == nullptr)
			return nullptr;
		else if (p->_clave == clave) {
			if (p->_dr == nullptr) {
				...
				return buscaMasIz(p->_dr);
			}
		}
		else if (p->_clave > clave)
			return buscaSigAux(p->_iz, clave, p);
		else
			return buscaSigAux(p->_dr, clave, ascendiente);
	}

	static Nodo * buscaMasIz(Nodo * p) {
		/*Nodo * aux = p;
		while (p != nullptr) {
			aux = p;
			p = p->_iz;
		...
		if (p == nullptr)
			return nullptr;
		else if (p->_iz != nullptr)
			return buscaMasIz(p->_iz);
		else
			return p;
	}
	/**
